...
function findLatestGenome(): string {
  ...
  for (const d of dirs) {
    ...
    for (const f of fs.readdirSync(d)) {
      if (/genome_.*\.json$/.test(f) || f === "simrunner_best_genome.json") {
        const p = path.join(d, f);
        candidates.push({ p, m: fs.statSync(p).mtimeMs });
      }
    }
  }
  if (!candidates.length) {
    throw new Error("No genome artifacts found. Provide one via --in.");
  }
  candidates.sort((a, b) => b.m - a.m);
  return candidates[0].p;
}

function resolveGenomePath(): string {
  if (inArg && fs.existsSync(inArg)) return inArg;
  if (inArg) console.warn(`Genome file not found: ${inArg}; searching for latest genome...`);
  return findLatestGenome();
}

const commit = execSync("git rev-parse --short HEAD").toString().trim();
const metaTag = `Version ${commit} — exported ${new Date().toISOString()}`;
...
function cgHeader(meta: string): string {
  return [
    "/**",
...
function makeHybridBot(tw: TW, meta: string): string {
  ...
  // esbuild outputs an IIFE wrapper which CodinGame rejects. Strip it to
  // expose plain top-level code matching the expected template format.
  if (code.startsWith("(() => {")) {
    code = code.replace(/^\(\(\) => {\n?/, "");
  ...
  // Prepend a brief comment header so the resulting file mimics the
  // boilerplate usually provided by CodinGame examples.
  code = [
    "/**",
...
(async function main(){
  if (mode === "genome") {
    const inPath = resolveGenomePath();
  ...
  if (mode === "hybrid") {
    ...
    writeOut(makeHybridBot(tw, metaTag));
    return;
  }
