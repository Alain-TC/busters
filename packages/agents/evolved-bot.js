...
export function act(ctx, obs) {
  if (obs.self.carrying !== undefined) {
    ...
    if (d <= 1506) return { type: "RELEASE" };
    return { type: "MOVE", x: ctx.myBase.x, y: ctx.myBase.y };
  }
  const enemy = obs.enemies?.[0];
  if (enemy && enemy.range <= 1676 && obs.self.stunCd <= 0) return { type: "STUN", busterId: enemy.id };
  ...
  if (!obs.self.radarUsed && obs.tick >= 6) return { type: "RADAR" };
  return { type: "MOVE", x: ctx.myBase.x, y: ctx.myBase.y };
}